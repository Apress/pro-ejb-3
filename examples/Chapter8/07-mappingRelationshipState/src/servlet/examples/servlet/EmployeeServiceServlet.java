...
public class EmployeeServiceServlet extends HttpServlet {
    ...
    private final String TITLE = 
        "Chapter 8: Using Relationships with state Example";
    
    private final String DESCRIPTION = 
        "This example demonstates the basics of mapping relationships that contain state.";
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        for (Employee emp : service.findAllEmployees()) {
            out.print(emp + " with " + emp.getAssignments().size() + " assignments<br/>");
        }
        
        out.println("</br>Projects: </br>");
        for (Project p : service.findAllProjects()) {
            out.print(p + " with " + p.getAssignments().size() + " assignments<br/>");
        }       
        printHtmlFooter(out);
    }
